...
import { ArrowRight, Star, Heart, ShieldCheck, Truck, Sparkles, RefreshCw } from 'lucide-react';
import { useCart } from '../context/CartContext';
import ScrollReveal from '../components/ScrollReveal';
import useSEO from '../hooks/useSEO';

export default function Home() {
  const { toggleWishlist, wishlist, addToCart, formatPrice, getProductsList } = useCart();
  ...
  useEffect(() => {
    const handleTyping = () => {
      ...
      if (isDeleting) {
        setDelta(50); // Deleting is twice as fast
      } else {
        setDelta(120); // Normal typing speed
      }

      if (!isDeleting && updatedText === fullText) {
        setDelta(2200); // Pause at end of word
        setIsDeleting(true);
      } else if (isDeleting && updatedText === '') {
        setIsDeleting(false);
        setLoopNum((prev) => prev + 1);
        setDelta(400); // Pause before next word
      }
    };
  ...
  useEffect(() => {
    getProductsList()
      .then(data => {
        // Grab first 4 items as featured
    ...
    <div className="space-y-16 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <ScrollReveal delay={0}>
            <div className="perspective-container">
              <Link to="/shop/beds" className="group relative rounded-premiumLarge overflow-hidden shadow-md aspect-[4/3] block border border-dreamBorder card-3d">
          ...
          <ScrollReveal delay={150}>
            <div className="perspective-container">
              <Link to="/shop/mattresses" className="group relative rounded-premiumLarge overflow-hidden shadow-md aspect-[4/3] block border border-dreamBorder card-3d">
                <div className="absolute inset-0 bg-dreamNavy/40 group-hover:bg-dreamNavy/30 transition-colors z-10"></div>
                <img 
          ...
          <ScrollReveal delay={300}>
            <div className="perspective-container">
              <Link to="/shop/accessories" className="group relative rounded-premiumLarge overflow-hidden shadow-md aspect-[4/3] block border border-dreamBorder card-3d">
                <div className="absolute inset-0 bg-dreamNavy/40 group-hover:bg-dreamNavy/30 transition-colors z-10"></div>
                <img 
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        <div className="flex items-end justify-between">
          ...
          <Link to="/shop" className="text-dreamAccent hover:text-dreamAccent-dark text-sm font-bold flex items-center gap-1">
