...
export default function Shop({ categoryFilter }) {
  ...
  // Local filter states (pre-populated by URL if present)
  ...
  // Sync state if categoryFilter prop changes (for Beds / Mattresses / Accessories pages)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    getProductsList()
      .then((data) => {
        ...
        // 3. Filter by Size (must match at least one variant size)
        if (size) {
          filtered = filtered.filter(p => p.variants && p.variants.some(v => v.size === size));
        }

        // 4. Filter by Min/Max Price (compares using base price)
        if (priceRange.min) {
  ...
  }, [category, search, size, priceRange, minRating, sort]);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
        ...
        >
          ...
          {/* Category Filter (hidden if pre-filtered on category pages) */}
          {!categoryFilter && (
                ...
                {[
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (c.key) newParams.set('category', c.key);
