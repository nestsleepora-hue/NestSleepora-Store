...
export default function Login() {
  ...
  const [failedAttempts, setFailedAttempts] = useState(() => {
    return parseInt(localStorage.getItem('sleepora_failed_attempts') || '0');
  });
  const [lockUntil, setLockUntil] = useState(() => {
    const stored = localStorage.getItem('sleepora_lock_until');
  ...
  const handleCredentialSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // Try to authenticate as admin first via the backend SQLite database (handles any admin added in Backoffice)
      ...
      try {
        ...
        // Not a valid admin or incorrect credentials, fall back to shopper flow
      }
      ...
      // Check browser trust cookie for shoppers
      ...
      // Simulate verification of credentials against the mock database
      ...
      // If we are not in mock mode (have actual firebase keys), we still verify locally or through Firebase
      // For this workflow, to enable 100% free OTP links, we do a preliminary verification
      if (!userExists && cleanEmail !== 'test@sleepora.com') {
        throw new Error('Auth/User not found or password incorrect');
      }
      
      // Verification passed -> generate and send OTP
  ...
  const handleOtpVerify = async (e) => {
    ...
    try {
      ...
      // Set device trust cookie for 30 days
      setDeviceTrustCookie(sanitizeInput(email));
  ...
  // Google Sign In (Google handles rate limit & OTP flows natively)
  ...
  // Shift focus logic inside 6-digit OTP fields
  ...
  // Forgot password workflow triggers
  ...
  return (
    <div className="min-h-screen lg:h-screen lg:overflow-hidden grid grid-cols-1 lg:grid-cols-12 bg-dreamBackground relative">
      ...
      {/* Left side: Premium brand visual pane (Visible on large screens) */}
      ...
      {/* Right side: Form pane (Centered on mobile/desktop) */}
              ...
              <>
                ...
                <div className="mt-4 pt-4 border-t border-dreamBorder text-center">
                  <p className="text-[11px] text-dreamMuted">
                    New to NestSleepora?{' '}
                    <Link to="/signup" className="text-dreamAccent hover:underline font-bold">
                      Create an account
                    </Link>
                  </p>
                </div>
              ...
              <div className="space-y-6 text-left">
                ...
                {dispatchedOtp && !pendingAdminSession && (
                  <div className="p-3 bg-dreamAccent/10 border border-dreamAccent/20 rounded-premium flex items-center gap-3 text-xs text-dreamAccent-dark font-semibold">
                    ...
                    <span>[Security Dispatcher] Test Code is: <strong>{dispatchedOtp}</strong> (Dispatched to: {email === 'mateen@itdepartment.com' ? 'mateen.soram@gmail.com' : email})</span>
                  </div>
                )}
      ...
      {/* ======================================================== */}
      {/* SECURITY DIALOG: Forgot Password OTP Reset Modal         */}
      {/* ======================================================== */}
      {showForgotModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-dreamNavy/60 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white border border-dreamBorder rounded-premiumLarge shadow-2xl overflow-hidden animate-fade-in relative">
            ...
            {/* Modal Header */}
            <div className="bg-dreamNavy text-white p-5 flex justify-between items-center">
