...
import { 
  ...
  Settings, X, PlusCircle, MinusCircle, Users, History, UploadCloud, LogOut, Video, Loader2, ArrowRight
} from 'lucide-react';
import ScrollReveal from '../../components/ScrollReveal';

export default function Admin() {
  const { 
    user, API_URL, formatPrice, getProductsList, getProductDetails, createProduct, 
    updateProduct, deleteProduct, getAuditLogs, logAdminAction,
    authLoading, forceUpdateAdminPassword, adminLogin, completeAdminLogin, verifyAdminOtp, checkDeviceTrustCookie, setDeviceTrustCookie
  } = useCart();
  ...
  // SSR Safe initialization of local storage user
  const [storedAdmin, setStoredAdmin] = useState(null);

  useEffect(() => {
    if (typeof window !== 'undefined') {
      setStoredAdmin(localStorage.getItem('sleepora_admin_user'));
    }
  }, []);
  ...
  if (user && user.role === 'admin' && user.must_change_password === 1) {
    return (
      <div className="min-h-screen bg-dreamBackground flex items-center justify-center px-4 py-12 font-sans">
        <ScrollReveal>
      ...
      <div className="max-w-md mx-auto px-4 py-24 text-center space-y-6 font-sans">
        <ScrollReveal>
          <div className="bg-white border border-dreamBorder rounded-premiumLarge p-8 shadow-xl flex flex-col items-center space-y-5">
            ...
            <a 
              href="/login" 
              className="px-6 h-11 bg-dreamNavy hover:bg-dreamNavy/90 text-white text-xs font-bold rounded-premium flex items-center justify-center transition-colors animate-fade-in-up"
            >
  ...
  const handleOpenEdit = async (prod) => {
    ...
    try {
      const fullProd = await getProductDetails(prod.id);
      const p = fullProd.product;
      
      setName(p.name);
      setDescription(p.description);
      setCategory(p.category);
      setPriceUsd(p.price_usd ? p.price_usd.toString() : p.base_price.toString());
      setPriceEur(p.price_eur ? p.price_eur.toString() : '');
      setPriceGbp(p.price_gbp ? p.price_gbp.toString() : '');
      setOriginalPriceUsd(p.original_price_usd ? p.original_price_usd.toString() : '');
      setOriginalPriceEur(p.original_price_eur ? p.original_price_eur.toString() : '');
      setOriginalPriceGbp(p.original_price_gbp ? p.original_price_gbp.toString() : '');
      setUploadedUrls(p.image_urls || []);
      setVariants(fullProd.variants || [{ size: 'Queen', price_modifier: 0, stock_qty: 10 }]);
      setProductReviews(fullProd.reviews || []);
    } catch (err) {
      console.error(err);
  ...
  const handleSaveReview = async (e) => {
    ...
    if (newReviewDate) {
      reviewDate = new Date(newReviewDate).toISOString();
    }
    ...
    try {
      if (editingReviewId) {
        ...
        await logAdminAction('UPDATE_REVIEW', `Updated review ID ${editingReviewId} on product "${name}" by "${newReviewAuthor.trim()}"`);
      } else {
        // Add mode
      ...
      // Reload product details from database to update variants and reviews
      const fullProd = await getProductDetails(editingProduct.id);
      setProductReviews(fullProd.reviews || []);
      ...
      // Refresh general products list in case ratings count changed
      fetchProducts();
    } catch (err) {
      console.error(err);
  ...
  const handleDeleteReview = async (reviewId) => {
    ...
    try {
      ...
      await logAdminAction('DELETE_REVIEW', `Deleted review ID ${reviewId} from product "${name}"`);

      // Reload product details from database
      const fullProd = await getProductDetails(editingProduct.id);
      setProductReviews(fullProd.reviews || []);
  ...
  return (
    <div className="min-h-screen bg-dreamBackground text-dreamNavy flex flex-col lg:flex-row relative font-sans">
        ...
        <div className="space-y-6 lg:space-y-8">
          <div className="flex items-center justify-between lg:block text-left">
            <div className="space-y-1">
              ...
              <h2 className="text-xl font-black font-poppins text-dreamNavy">Backoffice</h2>
          ...
          <div className="space-y-6">
            ...
            {loading ? (
              ...
              <div className="p-16 text-center bg-white border border-dreamBorder rounded-premiumLarge space-y-3">
                ...
                <p className="text-xs text-dreamMuted">Click "Add New Product" to populate database.</p>
              </div>
            ) : (
        ...
        {activeTab === 'logs' && isSuperAdmin && (
          <div className="space-y-6">
            ...
            {logsLoading ? (
              ...
              <div className="p-12 text-center bg-white border border-dreamBorder rounded-premiumLarge">
                <p className="text-sm text-dreamMuted">No admin activity logs logged.</p>
              </div>
            ) : (
              <div className="bg-white border border-dreamBorder rounded-premiumLarge overflow-hidden">
                <div className="overflow-x-auto scrollbar-none">
          ...
          <div className="w-full max-w-2xl bg-white border border-dreamBorder rounded-premiumLarge shadow-2xl overflow-hidden animate-fade-in relative max-h-[92vh] flex flex-col text-dreamNavy">
            ...
            <div className="p-6 overflow-y-auto space-y-5 text-left flex-1 scrollbar-none">
              ...
              <form onSubmit={handleFormSubmit} className="space-y-5">
                ...
                <div className="space-y-4 pt-2 border-t border-dreamBorder">
                  <h4 className="text-[10px] font-extrabold uppercase text-dreamAccent tracking-wider">Product Media Configuration</h4>

                  {/* Option A: Paste Online Image/Video URL */}
                  <div className="space-y-1.5 p-3.5 bg-dreamBackground rounded-premium border border-dreamBorder">
                    <label className="text-[10px] font-bold text-dreamNavy uppercase tracking-wider block">Option A: Add Image/Video URL link</label>
                    <div className="flex gap-2">
                      <input 
                        type="url"
                        placeholder="Paste online URL (e.g. https://i.ibb.co/your-image.jpg)"
                        value={inputUrl}
                        onChange={(e) => setInputUrl(e.target.value)}
                        className="flex-grow h-9 px-3 border border-dreamBorder rounded text-xs outline-none focus:border-dreamAccent bg-white text-dreamNavy focus:ring-1 focus:ring-dreamAccent"
                      />
                      <button 
                        type="button"
                        onClick={() => {
                          if (inputUrl.trim()) {
                            setUploadedUrls([...uploadedUrls, inputUrl.trim()]);
                            setInputUrl('');
                          }
                        }}
                        className="h-9 px-4 bg-dreamNavy text-white rounded text-[10px] font-bold hover:bg-dreamNavy/90 transition-colors"
                      >
                        Add URL
                      </button>
                    </div>
                  ...
                  {/* Option B: Local Files Upload */}
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-dreamNavy uppercase tracking-wider block">Option B: Upload media from local machine</label>
                  ...
                  <div className="space-y-4 pt-4 border-t border-dreamBorder">
                    ...
                    <div className="max-h-40 overflow-y-auto space-y-2 border border-dreamBorder p-3 rounded bg-dreamBackground scrollbar-none">
                      {productReviews.length === 0 ? (
                        ...
                        productReviews.map((rev, idx) => (
                          <div key={rev.id || idx} className="flex justify-between items-start bg-white p-2.5 rounded border border-dreamBorder/40 text-left">
                            ...
                            <div className="flex gap-2.5 shrink-0">
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  setNewReviewAuthor(rev.user_name);
                                  setNewReviewTitle(rev.title);
                                  setNewReviewRating(rev.rating);
                                  setNewReviewComment(rev.comment);
                                  const formattedDate = rev.created_at ? new Date(rev.created_at).toISOString().split('T')[0] : new Date().toISOString().split('T')[0];
                                  setNewReviewDate(formattedDate);
                                }}
                                className="text-[9px] text-dreamAccent hover:underline font-extrabold"
                              >
                                Edit
                              </button>
                              <button
                    ...
                    <div className="bg-dreamBackground p-4 rounded border border-dreamBorder space-y-3">
                      <div className="flex justify-between items-center">
                        <div className="text-[10px] font-bold uppercase text-dreamNavy">
                          {editingReviewId ? 'Edit Custom Review' : 'Add Custom/Seed Review'}
                        </div>
                        {editingReviewId && (
                          <button
                            type="button"
                            ...
                            className="text-[9px] text-dreamRed hover:underline font-extrabold uppercase"
                          >
                            Cancel Edit
                          </button>
                        )}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        ...
                        <div className="space-y-1">
                          <label className="text-[9px] text-dreamMuted uppercase">Review Date</label>
                          <input 
                            type="date" 
                            value={newReviewDate}
                        ...
                        <div className="space-y-1 flex flex-col justify-end">
                          ...
                          >
                            {reviewLoading ? 'Saving Review...' : editingReviewId ? 'Save Review' : 'Insert Custom Review'}
                          </button>
                        </div>
                      </div>
                ...
                <div className="pt-4 border-t border-dreamBorder flex justify-end gap-3">
                  ...
                  >
                    {formLoading ? 'Saving inventory...' : editingProduct ? 'Save Changes' : 'Push to Database'}
          ...
          <div className="w-full max-w-md bg-white border border-dreamBorder rounded-premiumLarge shadow-2xl overflow-hidden animate-fade-in relative flex flex-col text-dreamNavy">
            ...
            <div className="p-6 text-left">
              ...
              <form onSubmit={handleAddAdmin} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-dreamNavy uppercase">Full Name</label>
                  <input
                    type="text"
                    required
