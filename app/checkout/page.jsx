...
function CheckoutContent() {
  ...
  const isMockPaymentView = pathname.includes('/mock-payment') || mockSessionId;
    ...
    setTimeout(async () => {
      try {
        ...
        if (res.ok && data.success) {
          ...
          clearCart();
          setTimeout(() => {
            router.push(`/order-confirmation?session_id=${mockSessionId}`);
          }, 1000);
        } else {
  ...
  if (isMockPaymentView) {
    return (
      <div className="max-w-md mx-auto px-4 py-12 space-y-6 text-left font-sans">
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6 text-left font-sans">
...
export default function Checkout() {
  return (
    <Suspense fallback={
      <div className="max-w-md mx-auto px-4 py-20 text-center space-y-4 font-sans animate-pulse">
        <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-dreamAccent mx-auto"></div>
        <p className="text-xs text-dreamMuted">Loading secure checkout parameters...</p>
      </div>
    }>
      <CheckoutContent />
    </Suspense>
